...
import { ACCEPTED_FILE_TYPES, MAX_FILE_SIZE , UPLOAD_MAX_FILES, SUPPORTED_EXTENSIONS_STRING } from '@/constants';
import { UPLOAD_MESSAGES_CUS } from '@/constants/uploadcus';
...
export function FileUploadCus({ onFileSelect, onClear, onExport, onCreateBlank, isLoading, selectedFile, canExport, saveStatus }: FileUploadProps) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-300">
                <div {...getRootProps()} className={cn('cursor-pointer transition-all duration-200 hover:bg-linear-to-br hover:from-blue-50 hover:to-blue-100/50', isDragActive && 'bg-linear-to-br from-blue-50 to-blue-100/50 border-blue-400', isLoading && 'cursor-not-allowed opacity-50')}>
                    ...
                    <div className="flex flex-col items-center justify-center p-8 text-center">
                        ...
                        <h3 className="text-sm font-bold text-gray-900">{isDragActive ? UPLOAD_MESSAGES_CUS.titleActive : UPLOAD_MESSAGES_CUS.titleDefault}</h3>
                        <p className="text-xs text-gray-600 mt-1">{UPLOAD_MESSAGES_CUS.subtitle}</p>
                        <p className="text-xs text-gray-500 mt-0.5">{UPLOAD_MESSAGES_CUS.supportPrefix} {SUPPORTED_EXTENSIONS_STRING} (max {formatFileSize(MAX_FILE_SIZE)})</p>
                    </div>
                </div>
                <div onClick={onCreateBlank} className={cn('flex flex-col items-center justify-center p-8 text-center transition-all duration-200 hover:bg-linear-to-br hover:from-emerald-50 hover:to-emerald-100/50 cursor-pointer group', isLoading && 'cursor-not-allowed opacity-50')}>
                    ...
                    <p className="text-xs text-gray-600 mt-1 mb-4">Create a new file with 41 standard ERPNext Customer fields</p>
