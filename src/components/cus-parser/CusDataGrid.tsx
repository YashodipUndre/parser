...
import { DROPDOWN_OPTIONSCUS } from '@/constants/dropdowncus';
    ...
    if (isEditing && isDropdown) return (
        <Select value={editValue} onValueChange={save} onOpenChange={(o) => !o && setIsEditing(false)} open>
            ...
            <SelectContent>
                ...
                {DROPDOWN_OPTIONSCUS[columnName as keyof typeof DROPDOWN_OPTIONSCUS]?.map(o => <SelectItem key={o} value={o} className="text-sm font-medium">{o}</SelectItem>)}
            </SelectContent>
        </Select>
    );
    ...
    return (
        <tr key={row.id} style={{ height: `${vr.size}px` }} className={cn("border-b border-gray-100 hover:bg-blue-50/30", dup && "bg-orange-50/50")}>
            ...
            {row.getVisibleCells().map((c: Cell<LeadDataRow, unknown>) => {
                ...
                const isDropdown = !!DROPDOWN_OPTIONSCUS[c.column.id as keyof typeof DROPDOWN_OPTIONSCUS];
...
const HeaderCell = memo(({ column, k }: HeaderCellProps) => {
    const opts = DROPDOWN_OPTIONSCUS[k as keyof typeof DROPDOWN_OPTIONSCUS];
